...
export const Button = forwardRef(({ 
  ...
  return (
    <motion.button
      ...
      disabled={disabled}
      onClick={onClick}
      whileHover={variant !== 'ghost' && variant !== 'text' && !disabled ? { y: -1, scale: 1.01, transition: { duration: 0.18, ease: [0.16, 1, 0.3, 1] } } : {}}
      whileTap={!disabled ? { scale: 0.985, transition: { duration: 0.15, ease: [0.16, 1, 0.3, 1] } } : {}}
      className={cn(
        'inline-flex items-center justify-center rounded-none font-ui font-medium tracking-[0.05em] uppercase transition-colors duration-180 ease-out-expo select-none',
    ...
    >
      {variant === 'ghost' ? (
        <>
          {children}
          {/* Subtle text fade and underline slide for ghost buttons */}
          <motion.span 
